 ...
 * `react-easy-crop` itself uses to compute `cropPixels` -- deliberately not
 * `createImageBitmap`, whose `imageOrientation: "from-image"` mode can
 * report different (orientation-swapped) dimensions than the `<img>` the
 * crop box coordinates were computed against, making `drawImage` read a
 * rectangle outside the decoded bitmap and silently paint black. Staying on
 * the same element type as the crop UI keeps the two in sync.
 */
async function decodeImage(imageSrc: string): Promise<HTMLImageElement> {
...
export function ImageCropModal({
  ...
  /** Modal is open whenever this is non-null. Caller should pass an
      already HEIC-normalized, pre-shrunk file -- see the module doc. */
  file: File | null;
  aspect: number;
  ...
  async function onSave() {
    ...
    try {
      ...
      setError("That photo couldn't be read. Please try a different one (JPG, PNG, or WebP).");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Modal open={file !== null} onClose={handleCancel} title="Crop photo" size="lg">
      <div className="flex flex-col gap-3">
        {imageSrc && (
          // Fixed height, not flex-grow: react-easy-crop measures this
          // container's size on mount, and a size that comes from flex
          // layout inside a just-opened modal can be read as {0, 0} before
